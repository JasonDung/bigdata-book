...
```
[root@cdh1 opt]# yum install -y httpd
[root@cdh1 opt]# systemctl start httpd
```

* 从官网下载cdh源

```
[root@cdh1 opt]# wget https://archive.cloudera.com/cm6/6.2.0/repo-as-tarball/cm6.2.0-redhat7.tar.gz
[root@cdh1 opt]# sudo mkdir -p /var/www/html/cloudera-repos/cm6 
[root@cdh1 opt]# tar xvfz cm6.2.0-redhat7.tar.gz -C /var/www/html/cloudera-repos/cm6
[root@cdh1 opt]# sudo chmod -R ugo+rX /var/www/html/cloudera-repos/cm6
```

* 新增yum源配置文件

```
[root@cdh1 opt]# vi /etc/yum.repos.d/cloudera-repo.repo
[cloudera-repo]
name=cloudera-repo
## 这里配置httpd服务ip
baseurl=http://<web_server>/cloudera-repos/cm6/cm6.2.0
enabled=1
gpgcheck=0 
```

### 5.2，yum安装cloudera manager server
...
```
[root@cdh1 opt]# /opt/cloudera/cm/schema/scm_prepare_database.sh mysql -h mysql所在节点ip --scm-host 当前节点ip scm scm
Enter SCM password: 
JAVA_HOME=/usr/java/1.8
...
```
#创建用户
[root@cdh1 opt]# useradd -m cdh -G root -s /bin/bash 
#指定密码
[root@cdh1 opt]# passwd cdh
#配置用户root权限
[root@cdh1 opt]# visudo
# 输入  :98
...
### 5.7，其它节点安装cloudera manager server

* 将本地yum源文件发送至其它需要安装cdh的节点

```
[root@cdh1 opt]# cd /etc/yum.repo.d/
[root@cdh1 opt]# scp cloudera-repo.repo cdh2:$PWD
[root@cdh1 opt]# scp cloudera-repo.repo cdh3:$PWD
```

* 在cdh2、cdh3节点中安装agent，避免界面安装慢

```
[root@cdh1 opt]# yum install -y cloudera-manager-agent
```

* 离线下载parcels

注意：避免在线下载特别慢，从以下地址下载全部内容：
https://archive.cloudera.com/cdh6/6.2.0/parcels/

```
[root@cdh2 opt]#cd  /opt/cloudera/parcels
# 将下载好的parcels放入该目录下，并同步其它节点
[root@cdh2 parcels]# scp -r ./*  cdh3:$PWD
```
至此结束，其它的可在界面上操作！！
